...
const StudentBasket = ({ cities, defaultCities = [] }) => {
    ...
    useEffect(() => {
        if (defaultCities.length > 0 && selectedCities.length === 0) {
            const defaults = cities.filter(c => defaultCities.includes(c.city));
            setSelectedCities(defaults);
        }
    }, [defaultCities, cities, selectedCities.length]);
    ...
    const calculateBudget = (city) => {
        // 1. Rent
        let rent = 0;
        if (housingType === 'shared') rent = (city.apt_3bed_outside_center || 1000) / 3;
        else if (housingType === 'studio_out') rent = city.apt_1bed_outside_center || 800;
        ...
        const weeklyGrocery = (
            ...
            (city.potato || 1) + (city.tomato || 1) + (city.onion || 1)
        ); 
        ...
        const groceryFactor = Math.max(0, (mealsPerWeek - eatingOutFreq) / mealsPerWeek);
        const monthlyGrocery = weeklyGrocery * 4.3 * groceryFactor;
        const monthlyEatingOut = (eatingOutFreq * 4.3) * (city.meal_inexpensive || 15);
    ...
    const DonutChart = ({ data, size = 60 }) => {
        ...
        let cumul = 0;
        const slices = Object.entries(data).filter(([k]) => k !== 'total').map(([key, value]) => {
        ...
        return (
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                {slices.map((s, i) => <path key={i} d={s.d} fill={s.fill} stroke="#1e293b" strokeWidth="1"/>)}
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: 'minmax(300px, 1fr) 2fr', alignItems: 'start' }}>
            ...
            <div className="card" style={{position: 'sticky', top: '20px'}}>
                <div style={{display:'flex', alignItems:'center', gap:'10px', marginBottom:'1.5rem'}}>
                    <div className="icon-box" style={{background: 'var(--primary)', color:'#fff'}}>
                        <Utensils size={20} />
                    </div>
                    <div>
                        <h2 style={{margin:0, fontSize:'1.2rem'}}>Budget Persona</h2>
                        <div className="text-muted" style={{fontSize:'0.8rem'}}>Customize your student lifestyle</div>
                    </div>
                </div>
                ...
                <div className="config-section" style={{marginBottom: '1.5rem'}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.5rem'}}>
                        <span style={{fontWeight:'500', display:'flex', alignItems:'center', gap:'8px'}}><Home size={14}/> Housing Type</span>
                    </div>
                        ...
                        {['shared', 'studio_out', 'studio_in'].map((type) => (
                            <button 
                                ...
                                style={{
                                    ...
                                    border: 'none',
                                    borderRadius: '6px',
                                    background: housingType === type ? 'var(--primary)' : 'transparent',
                                    color: housingType === type ? '#fff' : '#94a3b8',
                                    cursor: 'pointer',
                                    transition: 'all 0.2s'
                                }}
                            >
                                {type === 'shared' ? 'Shared Room' : type === 'studio_out' ? 'Studio (Out)' : 'Studio (Center)'}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                {/* Eating Out Slider */}
                 <div className="config-section" style={{marginBottom: '1.5rem'}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.5rem'}}>
                        <span style={{fontWeight:'500', display:'flex', alignItems:'center', gap:'8px'}}><Utensils size={14}/> Eating Out (cheap)</span>
                 ...
                 <div className="config-section" style={{marginBottom: '1.5rem'}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.5rem'}}>
                        <span style={{fontWeight:'500', display:'flex', alignItems:'center', gap:'8px'}}><Beer size={14}/> Party Nights</span>
                ...
                {/* Coffee Slider */}
                 <div className="config-section">
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.5rem'}}>
                        <span style={{fontWeight:'500', display:'flex', alignItems:'center', gap:'8px'}}><Coffee size={14}/> Coffees</span>
             ...
             <div style={{display:'flex', flexDirection:'column', gap:'1rem'}}>
                <div style={{marginBottom:'1rem', display:'flex', gap:'10px'}}>
                    ...
                    />
                    {searchTerm && (
                        <div className="dropdown animate-fade-in" style={{
                            position: 'absolute', transform: 'translateY(50px)', width: '300px', 
                            background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius:'8px', zIndex: 50
                        }}>
                            {cities
                ...
                <div className="grid-cols-2" style={{gap:'1rem'}}>
                    {selectedCities.map(city => {
                        ...
                        return (
                             <div key={city.city} className="card animate-fade-in" style={{position:'relative', overflow:'hidden'}}>
                                <button 
                                ...
                                <div style={{display:'flex', alignItems:'center', gap:'15px', marginBottom:'1rem'}}>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-muted" style={{fontSize:'0.8rem'}}>{city.country}</div>
                                    </div>
                                </div>

                                <div style={{marginBottom:'1rem', borderTop:'1px solid rgba(255,255,255,0.1)', paddingTop:'10px'}}>
                                    <div className="text-muted" style={{fontSize:'0.8rem', textTransform:'uppercase'}}>Est. Monthly Need</div>
                 ...
                 {selectedCities.length === 0 && (
                     <div className="text-muted" style={{textAlign:'center', padding:'3rem', border:'2px dashed rgba(255,255,255,0.1)', borderRadius:'1rem'}}>
